...
const states = ['Maharashtra', 'Punjab', 'Uttar Pradesh', 'Karnataka', 'Tamil Nadu', 'West Bengal']
const cities = ['Mumbai', 'Pune', 'Nagpur', 'Ludhiana', 'Amritsar', 'Lucknow', 'Kanpur', 'Varanasi', 'Bangalore', 'Mysore', 'Chennai', 'Madurai', 'Kolkata', 'Siliguri']
const cropTypes = ['Wheat', 'Rice', 'Maize', 'Cotton', 'Sugarcane']
const seasons = ['Kharif', 'Rabi', 'Zaid']

export default function CropPriceForm() {
  const [formData, setFormData] = useState({
    date: new Date(),
    state: '',
    city: '',
  ...
  const handleSubmit = async(e) => {
    ...
    const formattedData = [
      ...
      parseFloat(formData.supply_volume),
      parseFloat(formData.demand_volume),
      parseFloat(formData.transport_cost),
      parseFloat(formData.fertilizer_usage),
      parseFloat(formData.pest_infestation),
      parseFloat(formData.market_competition)
    ]
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-100 to-blue-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-4xl">
        <h1 className="text-3xl font-bold text-center mb-6 text-green-600">Crop Price Data Form</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input type="number" step="0.1" id="temperature" name="temperature" value={formData.temperature} onChange={handleChange} required />
            </div>
            <div>
              <Label htmlFor="rainfall">Rainfall (mm)</Label>
              <Input type="number" step="0.1" id="rainfall" name="rainfall" value={formData.rainfall} onChange={handleChange} required />
            </div>
            <div>
              <Label htmlFor="supply_volume">Supply Volume</Label>
              <Input type="number" step="0.01" id="supply_volume" name="supply_volume" value={formData.supply_volume} onChange={handleChange} required />
            </div>
            <div>
              <Label htmlFor="demand_volume">Demand Volume</Label>
              <Input type="number" step="0.01" id="demand_volume" name="demand_volume" value={formData.demand_volume} onChange={handleChange} required />
            </div>
            <div>
              <Label htmlFor="transport_cost">Transport Cost</Label>
              <Input type="number" step="0.01" id="transport_cost" name="transport_cost" value={formData.transport_cost} onChange={handleChange} required />
            </div>
            <div>
              <Label htmlFor="fertilizer_usage">Fertilizer Usage</Label>
              <Input type="number" step="0.01" id="fertilizer_usage" name="fertilizer_usage" value={formData.fertilizer_usage} onChange={handleChange} required />
            </div>
            <div>
